...
const CustomButton: React.FC<CustomButtonProps> = ({ content, className, href, onClick }) => {
    return (
        <Link className={`relative px-6 py-2 group ${className}`} href={href || ''} onClick={onClick}>
            ...
            <span className="relative text-primary-custom group-hover:text-background-custom"> {content} </span>
        </Link>
)
}
